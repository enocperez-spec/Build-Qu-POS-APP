...
try {
    ...
    if ($action === 'latest') {
        if ($pdo) {
            ...
            if ($report && (!array_key_exists('qukdsVersions', $report) || !array_key_exists('quorbVersions', $report) || !array_key_exists('kioskStableVersion', $report['summary'] ?? []) || empty($report['summary']['trends']) || !array_key_exists('storeStatus', $report['downloadableVersions'][0]['storeRows'][0] ?? []) || !array_key_exists('storeBrands', $report['stores'][0] ?? [])) && !empty($latest['currentUploadId'])) {
